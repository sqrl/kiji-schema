 ...
 * HTableInterfaceFactory <i>must</i> return HTables.  We rely on this to test connectivity.
 *
...
public final class KijiHTablePool implements Closeable {
  ...
  public HTableInterface getTable() {
    try {
      ...
      throw new KijiIOException("Error attempting to receive a table from the pool.", e);
    }
  }
  ...
  public void close() throws IOException {
    ...
    try {
      ...
      throw new KijiIOException("Exception while attempting to close and clean-up pool.", e);
    }
  }
  ...
  class PooledHTableFactory extends BasePoolableObjectFactory<PooledHTable> {
    ...
    public PooledHTable makeObject() throws Exception {
      return new PooledHTable(mHTableFactory.create(mKiji.getConf(), mHBaseTableName));
    }

    @Override
    public void destroyObject(PooledHTable obj) throws Exception {
      obj.getWrappedTable().close();
    }
    ...
    public boolean validateObject(PooledHTable obj) {
      try {
        ...
        throw new KijiIOException("Coould not confirm state of " + obj.toString(), ioe);
      }
    }
  }
  ...
  public String toString() {
    return Objects.toStringHelper(this)
        .add("Table name", mTableName)
        .add("Table count", mPool.getNumActive() + mPool.getNumIdle())
        .toString();
  }
    ...
    try {
      ...
      if (!mPool.isClosed()) {
        ...
        if (CLEANUP_LOG.isDebugEnabled()) {
          CLEANUP_LOG.debug(
              "KijiHTablePool for '{}' was constructed through:\n{}",
              mTableName,
              mConstructorStack);
        }
        mPool.close();
